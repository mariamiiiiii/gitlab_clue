...
class CLUEAlgoGPU : public CLUEAlgo<T, NLAYERS> {
 ...
 public:
  ...
  // destructor
  ~CLUEAlgoGPU() { free_internal_data(); }
  ...
  void init_input_data() {
    ...
    CHECK_HIP_ERROR(
        hipMallocAsync(&d_points.x, sizeof(float) * reserve, stream_));
    CHECK_HIP_ERROR(
        hipMallocAsync(&d_points.y, sizeof(float) * reserve, stream_));
    CHECK_HIP_ERROR(
        hipMallocAsync(&d_points.layer, sizeof(int) * reserve, stream_));
    CHECK_HIP_ERROR(
        hipMallocAsync(&d_points.weight, sizeof(float) * reserve, stream_));
    if (useAbsoluteSigma_)
      CHECK_HIP_ERROR(
          hipMallocAsync(&d_points.sigmaNoise, sizeof(float) * reserve, stream_));
  }

  void init_output_data() { 
    unsigned int reserve = 1000000;
    CHECK_HIP_ERROR(
        hipMallocAsync(&d_points.rho, sizeof(float) * reserve, stream_));
    CHECK_HIP_ERROR(
        hipMallocAsync(&d_points.delta, sizeof(float) * reserve, stream_));
    CHECK_HIP_ERROR(hipMallocAsync(&d_points.nearestHigher,
                                     sizeof(int) * reserve, stream_));
    CHECK_HIP_ERROR(hipMallocAsync(&d_points.clusterIndex,
                                     sizeof(int) * reserve, stream_));
    CHECK_HIP_ERROR(
        hipMallocAsync(&d_points.isSeed, sizeof(uint8_t) * reserve, stream_));
  }

  void copy_todevice() {
    // input variables

    CHECK_HIP_ERROR(hipMemcpyAsync(d_points.x, points_.p_x,
                                     sizeof(float) * points_.n,
                                     hipMemcpyHostToDevice, stream_));
    CHECK_HIP_ERROR(hipMemcpyAsync(d_points.y, points_.p_y,
                                     sizeof(float) * points_.n,
                                     hipMemcpyHostToDevice, stream_));
    CHECK_HIP_ERROR(hipMemcpyAsync(d_points.layer, points_.p_layer,
                                     sizeof(int) * points_.n,
                                     hipMemcpyHostToDevice, stream_));
    CHECK_HIP_ERROR(hipMemcpyAsync(d_points.weight, points_.p_weight,
                                     sizeof(float) * points_.n,
                                     hipMemcpyHostToDevice, stream_));
    if (useAbsoluteSigma_)
      CHECK_HIP_ERROR(hipMemcpyAsync(d_points.sigmaNoise, points_.p_sigmaNoise,
                                       sizeof(float) * points_.n,
                                       hipMemcpyHostToDevice, stream_));
  }
  ...
  void copy_tohost() {
    ...
    CHECK_HIP_ERROR(hipMemcpyAsync(
        ...
        sizeof(int) * points_.n, hipMemcpyDeviceToHost, stream_));
    if (verbose_) {
      // other variables, copy only when verbose_==True
      CHECK_HIP_ERROR(hipMemcpyAsync(points_.rho.data(), d_points.rho,
                                       sizeof(float) * points_.n,
                                       hipMemcpyDeviceToHost, stream_));
      CHECK_HIP_ERROR(hipMemcpyAsync(points_.delta.data(), d_points.delta,
                                       sizeof(float) * points_.n,
                                       hipMemcpyDeviceToHost, stream_));
      CHECK_HIP_ERROR(hipMemcpyAsync(
          points_.nearestHigher.data(), d_points.nearestHigher,
          sizeof(int) * points_.n, hipMemcpyDeviceToHost, stream_));
      CHECK_HIP_ERROR(hipMemcpyAsync(points_.isSeed.data(), d_points.isSeed,
  ...
  void free_input_data() {
    CHECK_HIP_ERROR(hipFreeAsync(d_points.x, stream_));
    CHECK_HIP_ERROR(hipFreeAsync(d_points.y, stream_));
    CHECK_HIP_ERROR(hipFreeAsync(d_points.layer, stream_));
    CHECK_HIP_ERROR(hipFreeAsync(d_points.weight, stream_));
    if (useAbsoluteSigma_)
      CHECK_HIP_ERROR(hipFreeAsync(d_points.sigmaNoise, stream_));
  }

  void free_output_data() {
    CHECK_HIP_ERROR(hipFreeAsync(d_points.rho, stream_));
    CHECK_HIP_ERROR(hipFreeAsync(d_points.delta, stream_));
    CHECK_HIP_ERROR(hipFreeAsync(d_points.nearestHigher, stream_));
    CHECK_HIP_ERROR(hipFreeAsync(d_points.clusterIndex, stream_));
    CHECK_HIP_ERROR(hipFreeAsync(d_points.isSeed, stream_));
  }
 ...
 private:
  ...
  void clear_internal_buffers() {
    // // result variables
    CHECK_HIP_ERROR(hipMemsetAsync(d_points.rho, 0x00,
                                     sizeof(float) * points_.n, stream_));
    CHECK_HIP_ERROR(hipMemsetAsync(d_points.delta, 0x00,
                                     sizeof(float) * points_.n, stream_));
    CHECK_HIP_ERROR(hipMemsetAsync(d_points.nearestHigher, 0x00,
                                     sizeof(int) * points_.n, stream_));
    CHECK_HIP_ERROR(hipMemsetAsync(d_points.clusterIndex, 0x00,
                                     sizeof(int) * points_.n, stream_));
    CHECK_HIP_ERROR(hipMemsetAsync(d_points.isSeed, 0x00,
                                     sizeof(uint8_t) * points_.n, stream_));
    // algorithm internal variables
    CHECK_HIP_ERROR(
        hipMemsetAsync(d_hist, 0x00, sizeof(TilesGPU<T>) * NLAYERS, stream_));
    CHECK_HIP_ERROR(hipMemsetAsync(
